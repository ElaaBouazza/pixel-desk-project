...
class timer_entry extends Equatable {
  String title;
  String description;
  ...
  timer_entry({
    ...
    final today = DateTime.now(); //today dd/mm/yyyy, hh:mm:ss
    final newDate =
        DateTime.utc(today.year, today.month, today.day, 1, 0, 0, 0);//newdate is 21/01/04 1:0:0:0 or 1 hour

    this.currentTime = Duration.zero; //flutter class, currenttime is initialized to 0
    this.createdAt = createdAt != null ? createdAt : today;//if createdAT isn't a null date then it receives its value, else it's initialized to the date of today
    this.updatedAt = updatedAt != null ? updatedAt : today;
    this.history = List<Map<String, DateTime>>();//history is a list, intialized as an empty list
    this.timeLimit = timeLimit != null ? timeLimit : newDate; //the timelimit is 1 hour which ends in the same day, to remove because we don't need a limit
    this.reminderTime = reminderTime != null ? reminderTime : today;
  }

  timer_entry.fromJson(Map<String, dynamic> json) {
